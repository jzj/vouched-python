...
def test_int_jobs_submit_bad_date():
    ...
    with pytest.raises(InvalidRequestError) as e:
        job = client.submit(
            user_photo_path="/opt/app/tests/data/large.png",
            id_photo_path="/opt/app/tests/data/small.jpg",
    ...
    assert (
        e.value.errors[0].get("message")
        == "22/10/1970 is not a valid date in the format MM/DD/YYYY"
    )
    assert e.value.errors[0].get("type") == "InvalidRequestError"
...
def test_update_secret_client_key_large():
    ...
    client = Client(key)
    with pytest.raises(InvalidRequestError) as e:
        data = client.update_secret_client_key(
            secret_client_key="""
...
def test_int_jobs_submit_landscape():
    ...
    client = Client(key)
    job = client.submit(
        user_photo_path="/opt/app/tests/data/landscape.jpg",
        id_photo_path="/opt/app/tests/data/small.jpg",
        type="id-verification",
        callback_url="http://google.com",
        first_name="Thor Thunder",
