...
def toInvalidRequestError(e):
    errors = []
    if e.get("extensions").get("exception") and e.get("extensions").get(
        "exception"
    ).get("details"):
    ...
    errors = list(
        map(lambda x: dict(message=x.get("message"), type=x.get("type")), errors)
    )
...
def exception_to_error(e):
    ...
    if hasattr(e, "get") and e.get("extensions") and e.get("extensions").get("code"):
        if (
            e.get("extensions").get("code") == "BAD_USER_INPUT"
            and e.get("extensions").get("exception")
            and e.get("extensions").get("exception").get("details")
        ):
            return toInvalidRequestError(e)

        if e.get("extensions").get("code") == "BAD_USER_INPUT" and e.get(
            "extensions"
        ).get("code"):
            return toInvalidRequestError(e)
